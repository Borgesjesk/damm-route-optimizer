...
public class CarrierTrustService {
    ...
    public Carrier createCarrier(CarrierDTO dto) {
        ...
        int score = calculateTrustScore(dto.documentVerified(), dto.disputeCount(), false);
    ...
    public void validateCarrierForRoute(Long carrierId) {
        ...
        if (!carrier.isActive()) {
            throw new BusinessRuleException("Carrier is inactive and cannot be assigned to routes");
        }
        if (carrier.getTrustScore() < MIN_TRUST_SCORE_FOR_ROUTE) {
            throw new BusinessRuleException(
                String.format("Carrier '%s' has trust score %d/100 (minimum %d required). Level: %s. This carrier is blocked by FraudSentinel.",
                    carrier.getName(), carrier.getTrustScore(), MIN_TRUST_SCORE_FOR_ROUTE, carrier.getTrustLevel()));
        }
    }
    ...
    private int calculateTrustScore(boolean documentVerified, int disputeCount, boolean identityFlagged) {
        int score = 100;
